...
## v0.66 (2026-09-04) — The Field Moves at Your Pace, and Nothing Fails in Silence
Two small changes aimed at the two cheapest quits: the wait, and the mystery freeze.

**Battle speed — 1× / 2× / instant.** The enemy phase has always held a fixed beat on every acting foe (1.3s after each action lands, plus a 380ms wind-up before it moves, plus 400ms per staggered foe) — right for a first fight, brutal arithmetic by the time a ☠☠☠ field plus a revenant retinue puts a dozen bodies across the line: 20–30 seconds of watching, every round, growing exactly as your company does. A new button in the battle's floating controls (under the zoom keys, thumb-reach on a phone) cycles ▶ 1× → ▶▶ 2× → ⚡ instant, scaling every one of those beats and the walk glides through one throttle. Instant skips the *waiting*, never the *information*: a foe that only marches takes no time at all, but any action that draws blood holds a short beat so the damage numbers and hit flash land before the focus moves on — you always see what each blow did. The choice lives in the save and rides the battle stash, so it survives a reload mid-fight; old saves need nothing and read as 1×. While rebuilding the pacing chain, two of its latent seams got closed: the phase-advance timer now lives in one slot (two resume paths converging can no longer skip a foe), and an enemy is stamped per queue slot (a modal closing mid-phase against a still-pending retry can no longer make the same foe move twice).

**Nothing fails in silence.** There is no backend and no telemetry — by design. The cost was that a thrown exception in a click handler or the enemy-turn timer chain simply stalled the game with no trace: "it froze on my phone," unanswerable. Now any uncaught error (rejected promises included) raises a small dismissible banner naming the error and line, so a freeze becomes a report you can actually send. Banners cap at two and clear themselves; a clean game never shows one.

## v0.65 (2026-08-31) — Every ⚑ Answers to the Road, and Steel Learns Its Names
Two changes: one closes the last holes in "no work you can't reach", one opens the armory.

**No ⚑ beyond the road's end.** v0.64 gated the Quartermaster commissions (and every special posting) to provinces you've opened — but the *hex* inside the chosen region was still picked blind, and the road network wasn't consulted at all. Two ways that went wrong. First, terrain generation grows mountain blobs, and a blob can wall a pocket of perfectly pleasant plains off from every road (measured: about a quarter of fresh worlds carry at least one such pocket); a contract anchored there sat on the map answering "No road leads there" until it expired. Second, `travelTo` demanded a chokepoint *directly* between the caravan's province and the destination's — so after the reveal, a commission in a walked province two crossings away showed a working Travel button and then refused the click with "the realm does not fold that way." Both are gone. Contract anchors now draw only from the **known travel component** — a flood-fill of everywhere the caravan can be routed *today*, over opened ground — cached and recomputed only when a new province opens (the component only ever grows, so an anchor once valid stays valid). And the caravan's router is knowledge-aware: it threads multi-crossing marches through walked provinces, never routes wagons through ground you haven't answered for at a border, and when a route enters an unknown province it raises the crossing you'd actually arrive at — the Pilgrim Stair if you're coming through Karsk, not a shrug because the March shares no border with the Cinderlands. Brigand and horde spawns run through the same reachability sieve, so nothing in the world sits where the world can't get to it. Verified over 60 fresh realms with every province opened: 2,040 postings, all reachable by the same router the Travel button uses; the old code failed the identical probe.

**Forged affixes — prefixes temper the numbers, suffixes carry an ability.** Weapons and armor stop being interchangeable tiers. A blade can now come out of the world as a *Keen Arming Sword of Biting Steel*: six prefixes (Keen +10 hit · Tempered +1 damage · Cruel +2 on the high roll · Heavy and Brutal, damage bought with accuracy · Blessed vs the dead) and seven suffixes (Biting Steel ignores 2 armor · of the Wolf bites the wounded · of the Pyre burns the dead · of Warding guards its wielder · of the Vigil steadies resolve · of Long Flights extends a bow · of the Swift Hand quickens the feet), plus five armor tempers (Warded · Fleet · Grim · Padded against arrows · Thorned, which bleeds every melee blow its wearer takes — 1 damage, and it never kills). Where they come from: **battle salvage** rolls affixes scaling with the danger skulls (☠ 35%/15% prefix/suffix up to ☠☠☠ 65%/35%), harder fields can yield **armorer's work** worth stripping onto your own harness, the settlement armorer's **good batch** (1 in 5 fittings) comes tempered, and a **reforge** carries the old affixes into the new steel — with a 1-in-4 chance the smith's best work adds a name of its own. The dead keep theirs: a grave snapshot carries its affixes, so a revenant comes back swinging the named blade she died holding, and the Gear Echo hands it on with its temper intact. And none of it is free: every affix has a power price wired into the v0.58 budget, on your side and theirs — elite foes at ☠☠+ can spend their top-up budget on named steel, and a company draped in it buys itself a proportionally harder war. Old saves need nothing: soldiers without the new fields read as unadorned and play exactly as before.
...
Through Act I the view is cropped to the March, widened just enough that the neighbours' coastlines bleed off every edge where you can see them but never follow them. Then the King dies, Vell falls in the throne room, and the frame comes off: the map pulls back over 1.5 seconds to the whole of Vhal — **THE CINDERLANDS** above, **THE UNDERWAY** and **KARSK PROPER** and **THE BARROWLANDS** across the middle, **THE WESH REACH** and **THE THORNWOOD** below, and your March a small ringed square in the centre of it. Act I no longer ends the run; closing the chronicle there is now a choice you make, not one the game makes for you.

Fog became a three-rung ladder. Provinces bordering ground you've walked are **rumoured** — drawn in drained colour with their names legible — and word reaches you about them four times more often than about somewhere two crossings away. Everything else is a shape with `? ? ?` written on it. Rumour is stale and drifts by up to eight points; a garrison's report, or work you do on the border, is marked **seen** instead of **said**, because someone actually looked. New **Far Work** posts one contract at a time out of a province you've only heard of — fought on your own side of the crossing, paying 1.4×, and winning it buys true word of what's over there.

Under the hood the full continent costs **fewer** SVG nodes than v0.62's single province (791–933 against 950): the expensive part was never the hexes, it was a `<title>` and a click handler on each one. Non-focus provinces batch into one path per terrain, hex geometry is computed once, and the frame transition animates a wrapper rather than tweening the viewBox.

**Hardening.** A full adversarial audit of v0.62 turned up real damage, now fixed: `nearestSettlementHex` was reading province-local coordinates, so **every Blight horde in the game was frozen solid**; the weekly rot ran over all 31 regions from turn one and **swallowed the entire Barrowlands by week 10**, before the player was allowed to look at it; the Barrow Causeway joined two provinces that share no border, stamping its marker on the Treaty Gate's; the Act I barrow meeting could roll into a province you couldn't reach against a three-week deadline; the Blight-loss condition needed all 31 regions; deserters fled somewhere unreachable; and `hexDesc`/`hexTitle` crashed on realm-edge hexes the instant anything drew them. Migration is now guarded by sheet **shape** rather than a flag (a stripped flag used to turn the March into a 176-hex mountain slab), your save is backed up untouched before anything is rebuilt, and a failed load offers recovery instead of quietly starting a new company over your campaign.

## v0.62 (2026-07-16) — The Realm Opens: One March Among Seven
The camera pulls back. The March — the whole of Act I — turns out to be **one province of seven** on the continent of Vhal: Karsk Proper and its Crownseat, the Cinderlands where the Faith musters, the Underway's sealed dwarven gates, the Wesh Reach's smuggler coasts, the half-swallowed Barrowlands, and the Thornwood that has no towns at all. Thirty-one regions against Act I's six, on one continuous sheet the hordes and armies can cross.

**The realm exists from turn one — you just can't see it.** Provinces you have never walked are fogged: no hexes, no contracts, no truth. What you get instead is **rumour** — a carter's story, a drunk courier, word that arrives three weeks stale and drifted by up to eight points from the real number. The realm strip above the map shows each province and how old your information is, so you always know whether you're reading a fact or a story.

**Getting out is the whole problem.** Provinces are stitched together by eight named **chokepoints**, and each one is held by somebody who wants something: the Gate Road reads your letters (show Karsk standing, or buy papers that are almost genuine), the Deep Gate and the Treaty Gate charge tolls, the Barrow Causeway and the Rotting Weir are held by things that have to be fought through, and a couple of tracks are simply open because nobody has ever needed to guard them. Crossing a chokepoint for the first time is what opens a province — and only then does work start appearing on its board.

**Garrisons.** Leave a soldier holding a province and she sends true word every week instead of rumour, and grinds its Blight pressure down a little — but she cannot march while she does it. The twelve-soldier roster finally has to be *allocated*: the A-squad rides, the bench holds things.

Under the hood the map view windows onto the province you're standing in, so a world seven times larger still draws in about a millisecond — the same cost as the old six-region March. Existing campaigns migrate in place: your March is shifted into the middle of the continent with its terrain, blight, roads, settlements, agents, contracts and story markers exactly as you left them, and the other six provinces are built around it.

## v0.60 (2026-07-16) — Deeds & Name Levels: the Banneret and the Sworn
The first Realm Arc milestone: what happens past level 5. XP dies at veterancy — what comes after is earned by **Deeds**, ten earn-once acts detected from things the game already does (putting one of your own dead back in the ground, felling a Blight-Troll, bringing every charge home breathing, bleeding to the edge and walking back). Deeds are biography, not counters: they show on the roster card with the story attached, and most soldiers never earn enough to matter. Three deeds — **one of them hers alone** — and a town to hear them, and you can raise her banner.

**Level 6 — Banneret.** At a settlement, a ceremony: her deeds read out with their receipts, a bond-partner ties the first knot in the banner cord, and you choose the sword who kneels to it from three faces, each with a hook of her own ("owes the Undercrown a debt with interest", "buried a village and kept the bell-rope"). The sworn sword takes no muster slot — she is *hers, not yours* — and fights autonomously at her banneret's shoulder in a new **Retinue Step** between your turn and the enemy's, with a Guard/Loose stance. She earns no XP; she advances Green → Proven → Oathkept by surviving won fields at her side. When the banneret falls, her sword breaks or plants her feet over the body — and the swords who die beside her **rise with her**, by name and by face: *"Ysolt walks at her left hand still. 'Still Sworn' is not a title any more. It is a diagnosis."*

**Level 7 — Sworn.** Her name draws suitors. Oaths to the Banner, the Crown, or the Folk — offered by *her* history, not picked from a menu, with the perk **and** the strings printed before you click. The patron says exactly what it will ask ("The crown calls its sworn. It always does. Expect the rider."), and two weeks later it asks. Selling a body to the Conclave strains a Folk-sworn oath, and the rites screen warns you by name before you sell. At full strain she stands at the wagon-tongue and refuses to march — make it right, march one short, release her, or refuse her and make her Forsworn, which she wears on her card forever.

Every gift is a hook: sworn swords draw wages and eat, and they price into the enemy budget **unconditionally** — including a benched banneret's — so you can never promote your way into a cheaper fight.

## v0.59 (2026-07-16) — Difficulty Is Priced to Your Best, Not Your Choices
Follow-up to v0.58's correction from the field: the budget was reading the *deployed* squad's power, which quietly rewarded under-deploying — bring your B-team or a lone wounded soldier and the fight scaled down to match. That's backwards. Fielding a weak squad is a *strategic* decision, and it should be punished, not discounted. The budget now calibrates to the **strongest squad you *could* field** — the top `partyCap()` (4–6) soldiers in your roster by power — no matter who actually marches. Bench the A-team and the enemy is still priced to the A-team. (Current HP was never a factor either, so fielding the wounded was never a discount and still isn't.) The calibrated power is always ≥ any real deployment, so sub-optimal musters get harder, never easier. Verified: identical enemy budget whether you deploy your best six, your worst six, or one weakling.

## v0.58 (2026-07-16) — Power-Budget Encounters: Every Battle Prices Itself Off Your Company
The first slice of the Realm Arc roadmap. Enemy comps were static per danger rating — tuned for a fresh squad, so by level 3–4 the story, quartermaster, and escort battles (all on the weaker "flat" spawn path) had gone trivially easy while you kept growing. Now **every battle** — contracts, story set-pieces, ambushes, hordes — buys its enemies from a budget computed off your company's power (levels, weapon tier + mods, armor, feats). _(v0.59 refines the basis to your best possible muster rather than the squad actually deployed.)_ The danger skulls keep their meaning as *relative* difficulty: ☠ ≈ 0.5× your power (a day's work — comfortable, but it scales forever), ☠☠ ≈ 0.85× (earn your pay), ☠☠☠ ≈ 1.25× (the pressure test). Ratios are anchored so week-one battles are unchanged — the correction lands exactly where the flatness lived, from level 3 on. Count ceilings keep the skull identities legible (skirmish / warband / horde); budget the ceiling strands is spent making spawned foes *harder* — Veteran and Elite units with upgraded steel, capped per-unit so no husk becomes a god. Authored story comps always spawn whole, then reinforce with comp-consistent bodies (a bigger Coven is more husks, never a second Blight-Mage). Revenants and their retinues ride on top of the budget — the premium threat already scales itself. Measured: enemy power lands within 2% of target ratios at both fresh and maxed company strength, zero errors across ~500 spawn samples.

## v0.57 (2026-07-16) — Fog Hides Blight You Haven't Seen
Blight tiles were drawing their purple outline everywhere on the map — including across ground your squad had never laid eyes on — because the outline is a CSS `outline` (rendered outside the tile box), so neither the never-seen tile's black fill nor the explored-tile's dimming shadow covered it. The blight marker now respects fog of war: it shows only on tiles you can currently see or have already scouted, and dims along with the terrain on remembered ground. No more reading the blight map from the far dark corner your line-of-sight never reached.

## v0.56 (2026-07-16) — Revenants Come Back Stronger, and Not Alone
A revenant used to rise frozen at the level it died — so a soldier you lost early came back as a puny recruit against a company that had since grown, undercutting the whole "you will fight everyone you fail to protect" premise. Now a revenant **scales up to your company**: it comes back at roughly **+2 levels above your highest soldier** (and higher still each time the Conclave has improved her), grafting HP, skill, damage, and armor per level beyond its death-snapshot, gaining **Double Strike** at veterancy if it was a martial class, and filling out its old build along the class branch it favored in life. And it no longer walks alone — every revenant now leads a **Revenant Retinue**, its own pod detachment of 2–4 buffed risen dead (a Dread Graveguard bulwark, greataxe Husks, a Hound outrider on the deeper retinues), mustered around it and coordinating as a unit. The battle-start callout now reads the scaled threat: *"Level 6 now, grown past what you buried… 3 of the risen march at her back."*

## v0.55 (2026-07-16) — Enemy AI: Pathing Around Barriers & Dropping Unreachable Targets
Enemies used to steer by straight-line distance, so a wall, river, or ruin between them and their target would trap them — they'd shuffle into the barrier and waste the round, and two foes could sit stuck behind a wall the whole fight. The AI now navigates on a real **BFS distance field** that respects walls, cliffs, chasms and impassable ground (and refuses to cut diagonally through a wall's corner), so it routes *around* obstacles toward whatever it's chasing. And it no longer fixates: if the assigned target is walled off — or so far around a detour that something else is already in reach — the enemy **switches to the easier, closer target**. In practice a foe standing next to one of your soldiers, with its real objective sealed behind rubble, now just kills the soldier instead of pacing at the wall. Patrols and investigators use the same pathing, so they detour around terrain instead of stalling on it.
